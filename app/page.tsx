...
export default function Home() {
  ...
  useEffect(()=>{
    setClienteId(searchParams.get('cliente_id') || '206a762e-1038-4624-8772-ab4b9cd8b068')
  },[searchParams])
  ...
  const enviarLead = async ()=>{
    const {error} = await supabase.from('leads').insert([{
      name,email,phone,message,web_origen:'miweb',cliente_id:clienteId,codigo_afiliado:affiliateCode
    }])
    if(error){alert('Error')}
    else{
      setEnviado(true); setName(''); setEmail(''); setPhone(''); setMessage(''); setAffiliateCode('')
  ...
  return (
    <div style={{fontFamily:'sans-serif'}}>
      ...
      <div style={{padding:'30px',maxWidth:700,margin:'auto',textAlign:'center'}}>
        <p style={{fontSize:16,lineHeight:1.6}}>Diseñamos y desarrollamos páginas web con tecnología avanzada orientada a resultados, enfocadas en la captación de clientes reales para tu negocio.<br/><br/>No solo creamos tu web, implementamos sistemas de marketing digital que trabajan por ti, permitiéndote gestionar los contactos que entran, aumentar tus ventas y posicionarte de forma sólida en internet.</p>
      </div>
      ...
      <div style={{backgroundColor:'#0a2540',padding:'30px 20px',color:'white',textAlign:'center',marginTop:40}}>
        ...
        <p style={{fontSize:11,opacity:0.7,marginTop:20,lineHeight:1.5}}>Información legal: Los servicios ofrecidos corresponden a soluciones digitales personalizadas. Los resultados pueden variar en función de múltiples factores externos al servicio. Debido a la naturaleza del servicio y su activación inmediata, no se contemplan reembolsos una vez iniciado.</p>
      </div>
    </div>
  )
}